"""
This assessment is to test your knowledge of python dictionaries and functions and a lot more

- I used some advanced string formatting in the code below, check them out on Google to understand how the work and try
to use them in your code some other time
...
- The code below checks if  a user is logging in with the right information/details
- And shows the user balance
- A bank official can also log in
...
def login():
    ...
    user = int(input("Log in as a bank official or a bank user? Enter 1. for official 2. for user "))
    ...
    if user == 1:
        ...
        if password == bank_model["customers"][id]["password"]:
            msg = "Welcome back %s\nYour current balance: $%s" % (bank_model["customers"][id]["account_name"],
                                                                  bank_model['customers'][id]["balance"])
            print(msg)
